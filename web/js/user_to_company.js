...
var ViewUser = Backbone.View.extend({
	...
	template: _.template(	'<td class="u_fullname" rel="tooltip" data-placement="bottom" data-original-title="Double click for edit">'+
							...
							'<td class="u_company"><% print((company>0)?companies._byId[company].attributes.name:"undefined"); %></td>'+
							'<td class="u_email">'+
								'<%= email %> '+
	...
	edit: function() {
		...
		$('.u_company', this.el).html('<select name="company" class="company_update"></select>');
		
		var cid = this.model.get('company');
		
		companies.each(function(c){
			var view = new OptionCompaies({model:c});
			$('.company_update', this.el).append(view.render().el);
	...
	save: function() {
		...
		this.model.save({	fullname: $('.fullname', this.el).val(), 
							...
							company: $('.company_update', this.el).val(), 	
						},{wait: true});
	},
...
$(document).ready(function(){
	...
	$('#add_company_admin').click(function(){
		companies.each(function(p){
			var view = new OptionCompaies({model:p});
			$('.company_add').append(view.render().el);
		})
	})
})
